...
: "${MICROBENCH_BIN:?MICROBENCH_BIN required}"
: "${OUTDIR:?OUTDIR required}"
: "${MICROBENCH:?MICROBENCH required}"
...
# Microbench command selection
APP_CMD=()
case "$MICROBENCH" in
  llc_read_hit)
    LLC_HIT_WSS_MB="${LLC_HIT_WSS_MB:-2.0}"
    LLC_HIT_ITERS="${LLC_HIT_ITERS:-10000}"
    LLC_HIT_STRIDE="${LLC_HIT_STRIDE:-64}"
    APP_CMD=( "$MICROBENCH_BIN/llc_read_hit" --wss_mb "$LLC_HIT_WSS_MB" --iters "$LLC_HIT_ITERS" --stride "$LLC_HIT_STRIDE" )
    ;;
  pointer_chase)
    PTRCHASE_SIZE_MB="${PTRCHASE_SIZE_MB:-32.0}"
    PTRCHASE_STEPS="${PTRCHASE_STEPS:-500000000}"
    PTRCHASE_SEED="${PTRCHASE_SEED:-42}"
    APP_CMD=( "$MICROBENCH_BIN/pointer_chase" --size_mb "$PTRCHASE_SIZE_MB" --steps "$PTRCHASE_STEPS" --seed "$PTRCHASE_SEED" )
    ;;
  gather_scatter)
    GS_WSS_MB="${GS_WSS_MB:-32.0}"
    GS_REPEATS="${GS_REPEATS:-100}"
    GS_MODE="${GS_MODE:-both}"
    GS_LOCALITY="${GS_LOCALITY:-blocked}"
    APP_CMD=( "$MICROBENCH_BIN/gather_scatter" --wss_mb "$GS_WSS_MB" --repeats "$GS_REPEATS" --mode "$GS_MODE" --locality "$GS_LOCALITY" )
    ;;
  *)
    echo "[ERR] Unknown MICROBENCH=$MICROBENCH" >&2
    exit 20
    ;;
esac
[[ -x "${APP_CMD[0]}" ]] || { echo "[ERR] microbench binary not executable: ${APP_CMD[0]}" >&2; exit 21; }

# tmp
TMP_PARENT="${SLURM_TMPDIR:-${TMPDIR:-$OUTDIR/.tmp}}"
TMP_BASE="${TMP_PARENT%/}/sniper_${MICROBENCH}_${VARIANT}_${SLURM_JOB_ID:-nojob}_${SLURM_ARRAY_TASK_ID:-notask}"
...
LAUNCH=( env -i PATH="$PATH" LD_LIBRARY_PATH="${LD_LIBRARY_PATH:-}"
...
echo "[INFO] launching microbench: ${LAUNCH[*]}"
...
echo "[OK] MICROBENCH $MICROBENCH | ${L3_MB}MB | $VARIANT | $TECH -> $OUTDIR"
